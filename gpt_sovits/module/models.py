...
class TextEncoder(nn.Module):
    ...
    def forward(self, y, y_lengths, text, text_lengths, ge, speed=1, test=None):
        ...
        if speed != 1:
            y = F.interpolate(y, size=int(
                y.shape[-1] / speed) + 1, mode="linear")
...
class Quantizer(torch.nn.Module):
    ...
    def forward(self, xin):
        ...
        loss = 0.25 * torch.mean((z_q.detach() - xin) ** 2) + \
            torch.mean((z_q - xin.detach()) ** 2)
...
class CodePredictor(nn.Module):
    ...
    def forward(self, x, x_mask, refer, codes, infer=False):
        ...
        logits = x.reshape(x.shape[0], self.n_q - 1,
                           self.dims, x.shape[-1]).transpose(2, 3)
...
class CFM(torch.nn.Module):
    ...
    def inference(self, mu, x_lens, prompt, n_timesteps, temperature=1.0, inference_cfg_rate=0):
        ...
        for j in range(n_timesteps):
            t_tensor = torch.ones(
                x.shape[0], device=x.device, dtype=mu.dtype) * t
            d_tensor = torch.ones(
                x.shape[0], device=x.device, dtype=mu.dtype) * d
...
class SynthesizerTrnV3(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cfm = CFM(
            100,
            DiT(**dict(dim=1024, depth=22, heads=16, ff_mult=2,
                text_dim=inter_channels2, conv_layers=4)),
...
class SynthesizerTrnV3b(nn.Module):
    ...
    def decode_encp(self, codes, text, refer, ge=None):
        ...
        if ge == None:
            ...
            ge = self.ref_enc(refer[:, :704] * refer_mask, refer_mask)
        y_lengths = torch.LongTensor([int(codes.size(2) * 2)]).to(codes.device)
        y_lengths1 = torch.LongTensor(
            [int(codes.size(2) * 2.5 * 1.5)]).to(codes.device)
        text_lengths = torch.LongTensor([text.size(-1)]).to(text.device)
...
class BigVGANGenerator(nn.Module):
    def __init__(self, config):
        ...
        self.num_kernels = config["resblock_kernel_sizes"]
        self.num_upsamples = len(self.upsample_rates)
        self.conv_pre = nn.utils.weight_norm(
            nn.Conv1d(config["num_mels"],
                      config["upsample_initial_channel"], 7, 1, padding=3)
        )

        # 你可以根據需要繼續補全 BigVGAN 的模型結構（若用的是簡化模型可能只需這段）

    def forward(self, x):
        # 假設用預訓練推理，只要 forward 傳進 mel，回傳 fake audio
        return x  # 這裡只是佔位，實際推理的 bigvgan_generator.pt 會覆蓋掉 forward
